...
turtle.left(40) # поворот налево в определенном угле
turtle.dot() # рисуем точку
turtle.penup() # поднимаем перо (перестаем рисовать)
turtle.forward(100)
turtle.right(60) # поворот направо в определенном угле
...
# turtle.reset()
# все стирается, задается черный цвет пера, черепаха возвращается в центр координат, цвет фона не изменяется

# turtle.clear()
# просто все стирается
...
turtle.forward(100)
turtle.end_fill() # конец заливки
# если фигура не замкнута, то заливается только какая-то часть
turtle.clearscreen()
